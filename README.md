...
> [!WARNING]  
> Vanthasix 14 is not compatible with `settings.json` files from v13 or earlier. You can keep the same `database.sqlite` though without having any issues.

Vanthasix is a high-performance client area for the Pterodactyl Panel. It allows your users to create, edit and delete servers, and also earn coins which can be used to upgrade their servers.

## Get started

You can get started straight away by following these steps:

1. Clone the repo: Run `git clone https://github.com/Vanthasixoss/Vanthasix` on your machine
...
3. Check everything out and make sure you've configured Vanthasix correctly
...
```
npm run start - starts Vanthasix via nodemon
npm run build - builds TailwindCSS, required for making changes to the UI
```

## Vanthasix API v2

In v14, we've introduced the next generation of Vanthasix's API. You can see the documentation below:
